...
# run applications
function runApp() {
  echo "INFO: $FLINK run -c ${JOB} ${JAR} -request-rate ${REQUEST_S} -p-state ${STATE_P} -p-filter ${FILTER_P} -state-size ${STATE_SIZE} -keys ${KEY_SIZE} -group-all ${Group} -skewness ${SKEWNESS} -hist-file ${FILE_PATH} -cp-interval ${CP_Interval} &"
  rm nohup.out
  nohup $FLINK run -c ${JOB} ${JAR} -request-rate ${REQUEST_S} -p-state ${STATE_P} -p-filter ${FILTER_P} -state-size ${STATE_SIZE} -keys ${KEY_SIZE} -group-all ${Group} -skewness ${SKEWNESS} -hist-file ${FILE_PATH} -cp-interval ${CP_Interval} &
}
