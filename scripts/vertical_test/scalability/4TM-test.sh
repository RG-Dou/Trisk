...
#  query (1), parallelism (2)，total memory (3), state_size (4), key_size(5), controller (6), group (7), source_rate (8), try_counter (9), input_spy(10)
  bash nexmark-run.sh q4 ${PP} 350 100000 ${key_size_nex} TestInitMemoryManager true 500 1 false
  bash nexmark-run.sh q20 ${PP} 350 100000 ${key_size_nex} TestInitMemoryManager true 500 1 false

# parallelism (1)，total memory (2), state_size(3), controller (4), group (5), source_rate (6), try_counter (7)
  bash lr-run.sh ${PP} 500 ${state_size_DE} TestInitMemoryManager true 500 1 false
}
